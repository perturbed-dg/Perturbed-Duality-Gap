...
import sys
from dataio.dataset_2D import Dataset_2D
import utils
...
class Gan(Basic_model):
    ...
    def _load_datasets(self):
        config = self.config
        self.train_dataset = Dataset_2D()
        self.train_dataset.load_data(config.data_task,config.data_dir,
...
#-----------------------------------------------------------------------------------
    ...
    def evaluate(self, step,action_list,trial=0):
        ...
        self.kde(samples.T,save=img_save_path)
        fake_kernel = stats.gaussian_kde(samples.T)
        real_kernel = stats.gaussian_kde(X.T)
        xx, yy = np.mgrid[bbox[0]:bbox[1]:300j, bbox[2]:bbox[3]:300j]
        positions = np.vstack([xx.ravel(), yy.ravel()])
        ...
        print(' \n KL DIVERGENCE : ',kl)
        pd.DataFrame.from_dict(kl_data).to_csv('{}/KL.csv'.format(kl_save_path),header=False,index=False)
    ...
    def generate_images(self, step):
        ...
        samples    = self.generator(self.fixed_noise_128).numpy()
        checkpoint_dir = os.path.join(self.config.save_images_dir,self.config.data_task, self.exp_name)
        
        if not os.path.exists(checkpoint_dir):
            os.makedirs(checkpoint_dir)
        save_path = os.path.join(checkpoint_dir, 'images_{}.png'.format(step))
        r, c = 1, 2
        
        fig, axs = plt.subplots(r, c,figsize=(16,8))
        
        X = self.train_dataset.sample_batch(self.config.vis_count)
        axs[0].set_title('Learning Visualization')
        axs[0].scatter(X[:,0],X[:,1],color='r',alpha=0.11,label='Real Data')
        axs[0].scatter(samples[:,0],samples[:,1], color='b',alpha=0.1,label='Fake Data')
        
        axs[1].set_title('Duality Gap')
        x_axis = [i*self.config.valid_frequency_task for i in range(len(self.dg_values))]
        axs[1].plot(x_axis,self.dg_values)

        tf.summary.image('Generated Images',plot_to_image(fig),step=self.step_number)
